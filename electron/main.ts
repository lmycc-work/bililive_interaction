...
//
//
//   // ipcMain.on('send-live-data-to-child', (_, windowKey, data) => {
//   //   const childWindow = childWindows.get(windowKey)
//   //   if (childWindow && !childWindow.isDestroyed()) {
//   //     childWindow.webContents.send(`data-for-${windowKey}`, data) // 发送到子窗口
//   //     console.log(`✅ 精准推送数据到 ${windowKey}`)
//   //   }
//   // })
//   // ========== 唯一的消息转发逻辑：精准定向（无广播） ==========
...
//       // 2. 向目标窗口发送「专属消息」，仅该窗口能收到
//       targetWin.webContents.send(`data-for-${targetWindowKey}`, data)
//       console.log(`✅ 精准推送数据到 ${targetWindowKey}（仅一次）`)
//     } else {
//       console.warn(`⚠️ 窗口 ${targetWindowKey} 不存在，跳过推送`)
//     }
//   })
//
//
...
ipcMain.on('send-data-to-child', (_, targetWindowKey, data) => {
  ...
  if (targetWin && !targetWin.isDestroyed()) {
    ...
    console.warn(`⚠️ 窗口 ${targetWindowKey} 不存在或已关闭，跳过推送`)
  }
})
